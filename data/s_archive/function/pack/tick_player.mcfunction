...
execute if entity @s[tag=struct_check] run return run function s_archive:structure/tick_check
...
execute if entity @s[tag=returning_from_archive] run return run function s_archive:archive/return_from_archive

execute as @e[type=zombie,distance=..100,tag=!caveman_rng] run function s_archive:items/real/cave_luck

execute as @e[type=item_display,distance=..100,tag=trophy_dis] at @s if block ~ ~-1 ~ air run function s_archive:items/trophy/tick_break

stopsound @s[tag=stop_records] record
tag @s[tag=stop_records] remove stop_record